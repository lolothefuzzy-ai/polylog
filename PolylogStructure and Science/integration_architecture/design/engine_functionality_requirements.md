# Engine Functionality Requirements & Resource Guide
...
|-----------|-----------|--------|----------|
| Guardrail Integration | `simulation/engines/guardrails.py` | ✅ Complete | Dimension-aware evaluation merged; regression suite green |
| Catalog Scaffolding | `scripts/generate_*.py`, `catalogs/` | ✅ Complete | All four generators emit representative data |
| Catalog Loading | `simulation/placement/runtime.py` | ✅ Ready | `CatalogBackedHydrator` instantiates with version validation |
| Storage Regression | `storage/tests/test_storage_pipeline.py` | ✅ Extended | Mega-scale fixtures (128–8192 polygons) parametrized |
...
### Gap 1 — Placement Runtime Attachment Resolution
...
- scoring heuristic: `stability_score × closure_fit × collision_gate`
...
3. Add fixtures consuming `attachment_graph.json` triangle↔triangle and triangle↔square entries.

---

### Gap 2 — Combinatorial Calculator (O/I Metrics)
...
class CombinatorialCalculator:
    ...
    def calculate_O(self, assembly, method: str = "auto") -> float:
        ...  # cascading → enumeration → estimation
...
### Gap 3 — Fold Sequencer (Instantaneous Folding)
...
### Gap 4 — Collision Detection Validation
...
1. **Unit Tests**  
   - `tests/test_placement_with_catalog.py` – Attachment resolver hooks.  
   - `tests/test_fold_sequencer.py` – Angle validation & collision gating.  
   - `tests/test_combinatorial_calculator.py` – O/I correctness.

2. **Integration Tests**  
   - `tests/test_placement_runtime_integration.py` – End-to-end placement cycle.  

3. **Regression Tests**  
   - `storage/tests/test_storage_pipeline.py` – Chunking parity.  
   - `simulation/tests/test_guardrails.py` – Guardrail stability.  

4. **Performance Benchmarks**  
   - `tests/test_placement_performance.py` – < 100 ms per placement target.
...
- [ ] I formula reproduces tetrahedron expectations (≈ 7).  
...
**Priority Path A – Main Integration**
...
**Path B – Easy Wins (Self-Isolated)**
